...
read -p "请输入→：：" h
[ -z "$h" ] && h=qq.com
echo -e \\n"\e[1;36m↓↓ 請输入本地监听端口 b，留空或回车则默认使用56666端口。注意放行端口防火墙入站↓↓\e[0m"
read -p "请输入→：" b
[ -z "$b" ] && b=56666
echo -e \\n"\e[1;36m↓↓ 請输入内网转发IP t，留空或回车则默认使用本机127.0.0.1↓↓\e[0m"
read -p "请输入→：" t
[ -z "$t" ] && t=127.0.0.1
echo -e \\n"\e[1;36m↓↓ 請输入内网转发端口 p，留空或回车则默认使用端口12345↓↓\e[0m"
read -p "请输入→：" p
[ -z "$p" ] && p=12345
#自启
...
[ -z "$p" ] && p=12345 && echo "p=12345" >> $dirconf/settings.txt
		...
		if [ "$size" -eq "0" ] ; then
			...
			if [ "$dirsize" -eq "0" ] ; then
				if [ "$size" -lt "1024" ] ; then
					echo "！！检测到$diretc剩余空间$size KB小于 1 MB，强制将资源文件夹改为 $diretc_new"
					sed -i '/diretc=/d' $dirconf/settings.txt
					diretc=$diretc_new
					echo "diretc=$diretc" >> $dirconf/settings.txt
				fi
...
pss='ps -w |grep -v grep| grep "${name} "'
pid='pidof ${name}'
...
if [ ! -z "$i" ] ; then
	echo -e "★ \e[1;36m 网卡 i：\t\e[1;32m $i\e[0m"
else
	echo -e "☆ \e[1;36m 网卡 i：\t\e[1;31m【不存在】\e[0m"
fi
if [ ! -z "$s" ] ; then
	echo -e "★ \e[1;36m STUN服务器 s：\e[1;32m $s\e[0m"
else
	echo -e "☆ \e[1;36m STUN服务器 s：\e[1;31m【不存在】\e[0m"
fi
if [ ! -z "$h" ] ; then
	echo -e "★ \e[1;36m HTTP服务器 h：\e[1;32m $h\e[0m"
else
	echo -e "☆ \e[1;36m HTTP服务器 h：\e[1;31m【不存在】\e[0m"
fi
if [ ! -z "$b" ] ; then
	echo -e "★ \e[1;36m 本地监听端口 b：\e[1;32m$b\e[0m"
else
	echo -e "☆ \e[1;36m 本地监听端口 b：\e[1;31m【不存在】\e[0m"
fi
if [ ! -z "$t" ] ; then
	echo -e "★ \e[1;36m 转发内网地址 t：\e[1;32m$t\e[0m"
else
	echo -e "☆ \e[1;36m 转发内网地址 t：\e[1;31m【不存在】\e[0m"
fi
if [ ! -z "$p" ] ; then
	echo -e "★ \e[1;36m 转发内网端口 p：\e[1;32m$p\e[0m"
else
	echo -e "☆ \e[1;36m 转发内网端口 p：\e[1;31m【不存在】\e[0m"
fi
pss_status=$(pss | wc -l)
