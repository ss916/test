...
run="$dirtmp/${name} -c $dirtmp/${name}.json"
...
[ ! -s ./${name}_ver.txt ] && echo "▶查询主程序$name 版本号..." && echo "$(version)" | sed '/^ *$/d' | awk '/version/{print $3}' > ./${name}_ver.txt
...
if [ -s /tmp/$configfile -o -s $dirconf/$configfile ] ; then
	if [ -s /tmp/$configfile ] ; then
		...
		logger -t "【${bashname}】" "▶进入测试模式，使用本地配置文件/tmp/$configfile" && echo -e \\n"\e[1;36m▶进入测试模式，使用本地配置文件/tmp/$configfile\e[0m"
	elif [ -s $dirconf/$configfile ] ; then
		cp -f $dirconf/$configfile ./$configfile
		#ver=$(cat ./$configfile | awk -F// '/【/{print $2}')
		logger -t "【${bashname}】" "▶使用闪存配置文件$dirconf/$configfile" && echo -e \\n"\e[36m▶使用本地配置文件$dirconf/$configfile\e[0m"
	fi
else
...
wait
echo -e \\n"\e[1;33m...更新完成...\e[0m"\\n
[ -s ./$name ] && echo "$(version)" | sed '/^ *$/d' | awk '/version/{print $3}' > ./${name}_ver.txt
exit
}
...
if [ -s /tmp/${name}.json ] ; then
	echo -e "★ \e[1;36m ${name} 配置：\e[1;32m【${name}.json】\e[0m临时"
elif [ -s $dirconf/${name}.json ] ; then
	echo -e "★ \e[1;36m ${name} 配置：\e[1;32m【${name}.json】\e[0m闪存"
else
...
if [ ! -z "$(port)" ] ; then
	echo -e "● \e[1;36m ${name} 端口：\e[1;32m【已监听】\e[0m listen \e[1;37m$(awk -F '"' '/localaddr/{print $4}' $dirconf/${name}.json)\e[0m"
else
