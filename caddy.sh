...
#初始化settings.txt
settings () {
echo -e \\n"\e[1;36m↓↓ 初始化设置，請輸入启动模式mode，默认1↓↓\e[0m"\\n
read -p "请输入mode：" mode
echo -e \\n"\e[1;36m↓↓ 请输入是否下载clash_web文件，1开启，0关闭。默认0↓↓\e[0m"\\n
read -p "请输入是否下载clash_web文件：" clash_web
echo -e \\n"\e[1;36m↓↓ 请输入是否下载Country.mmdb文件，1开启，0关闭。默认0↓↓\e[0m"\\n
read -p "请输入是否下载Country.mmdb文件：" country_mmdb
#自启
...
[ -z "$country_mmdb" ] && country_mmdb=0 && echo "country_mmdb=$country_mmdb" >> $dirconf/settings.txt
...
if [ ! -z "$2" -a ! -z "$3" ] ; then
	#一键快速设置参数：./caddy.sh 1 wan=1 cron=1
...
run="$dirtmp/${name} run --config $dirtmp/Caddyfile"
...
version="$dirtmp/${name} version | sed 's/v//;s/ .*//'"
...
#设置环境变量
#export HOME="$dirtmp" #主目录
#export XDG_DATA_HOME="$dirconf" #证书保存目录
...
file=c
if [ "$secret" = "1" ] ; then
	downloadfile address=s/$file filetgz=$file decrypt=1 password=$password fileout=./ filename=Caddyfile
else
	downloadfile address=s/$file filetgz=$file fileout=./ filename=Caddyfile
fi
}
...
if [ -s /tmp/$configfile -o -s $dirconf/$configfile ] ; then
	...
	echo -e \\n"\e[1;31m✖『$configfile』配置文件不存在，结束脚本。\e[0m"\\n
	exit
fi
}
...
if [ -s /tmp/Caddyfile ] ; then
	echo -e "★ \e[1;36m ${name} 配置：\e[1;32m$(cat /tmp/Caddyfile | awk -F// '/【/{print $2}')\e[0m临时"
elif [ -s $dirconf/Caddyfile ] ; then
	echo -e "★ \e[1;36m ${name} 配置：\e[1;32m$(cat $dirconf/Caddyfile | awk -F// '/【/{print $2}')\e[0m闪存"
else
