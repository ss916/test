...
read -p "请输入→：：" server_key
echo -e \\n"\e[1;36m↓↓ 請输入本地listen监听端口，按回车则默认使用7000端口↓↓\e[0m"
read -p "请输入→：" listen
echo -e \\n"\e[1;36m↓↓ 請输入fec发包倍率，按回车则默认使用2:2↓↓\e[0m"\\n
read -p "请输入→：" fec
#自启
...
[ -z "$server_domain" -o -z "$server_port" -o -z "$server_key" ] && echo -e "\e[1;31m✖服务器server_domain:[$server_domain]、server_port:[$server_port]、server_key:[$server_key] 其一参数为空，取消启动。\e[0m" && exit
server_ip=$(nslookup $server_domain 208.67.222.222:5353 | sed -n '/Name/,$p' | grep -E -o '([0-9]+\.){3}[0-9]+' | head -n 1)
[ -z "$server_ip" ] && echo -e "\e[1;31m✖服务器域名$server_domain解析DNS为空，取消启动。\e[0m" && exit
...
if [ -z "$(echo $new_fec | grep -E '^[0-9]+:[0-9]+$')" ] ; then
	echo " ✖ [$new_fec] 输入的格式不正确，请重新输入。格式： 【原发包数字:多发包数字】"
else
	...
	if [ "$z" -gt "50" ] ; then
		...
		if [ ! -z "$(pid)" ] ; then
			...
			echo "! $name 进程未启动，跳过设置动态 fec"
		fi
		[ "$new_fec" != "$fec" ] && echo ">> 修改或添加参数fec=[$new_fec]到配置文件settings.txt" && sed -i '/^fec=/d' $dirconf/settings.txt && echo "fec=$new_fec" >> $dirconf/settings.txt
	fi
fi
}
...
if [ ! -z "$fec" ] ; then
	echo -e "★ \e[1;36m 发包倍率fec：\e[1;32m$fec\e[0m"
else
	echo -e "☆ \e[1;36m 发包倍率fec：\e[1;31m【不存在】\e[0m"
fi
pss_status=$(pss | wc -l)
...
if [ ! -z "$(port)" ] ; then
	echo -e "● \e[1;36m ${name} 端口：\e[1;32m【已监听】\e[0m listen：\e[1;37m$listen\e[0m"
else
