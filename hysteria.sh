...
url2="https://ghproxy.com/https://raw.githubusercontent.com/ss916/test/main"
url3="https://raw.githubusercontents.com/ss916/test/main"
url4="https://rrr.ariadl.eu.org/ss916/test/main"
url5="https://fastly.jsdelivr.net/gh/ss916/test@main"
url6="https://gcore.jsdelivr.net/gh/ss916/test@main"
...
#初始化settings.txt
settings () {
echo -e \\n"\e[1;36m↓↓ $name初始化设置，請輸入服务器IP或域名 server↓↓\e[0m"\\n
read -p "请输入→：" server_domain
echo -e \\n"\e[1;36m↓↓ 請输入服务器端口port↓↓\e[0m"\\n
read -p "请输入→：" server_port
echo -e \\n"\e[1;36m↓↓ 請输入服务器认证密码auth↓↓\e[0m"\\n
read -p "请输入→：" auth
echo -e \\n"\e[1;36m↓↓ 請输入服务器混淆密码obfs，无则按回车跳过↓↓\e[0m"\\n
read -p "请输入→：" obfs
echo -e \\n"\e[1;36m↓↓ 請输入传输协议transport，按回车则默认使用udp↓↓\e[0m"\\n
read -p "请输入→：" transport
echo -e \\n"\e[1;33m★ 以下为高级设置参数，不清楚参数作用请直接按回车跳过。\e[0m"\\n
echo -e \\n"\e[1;36m↓↓ 請输入本地socks5监听端口，按回车则默认使用9005端口↓↓\e[0m"\\n
read -p "请输入→：" listen_socks5
echo -e \\n"\e[1;36m↓↓ 請输入tproxy透明代理端口，按回车则默认使用9001端口↓↓\e[0m"\\n
read -p "请输入→：" listen_tproxy
echo -e \\n"\e[1;36m↓↓ 請输入宽带的最大上行带宽up，按回车则默认使用20 mbps↓↓\e[0m"\\n
read -p "请输入→：" up
echo -e \\n"\e[1;36m↓↓ 請输入宽带的最大下行带宽down，按回车则默认使用100 mbps↓↓\e[0m"\\n
read -p "请输入→：" down
#自启
...
read -n 1 -p "请输入：" cron
###
echo -e \\n"\e[1;37m你输入了：\\n\\n服务器域名domain：$server_domain \\n服务器端口port：$server_port \\n服务器混淆密码obfs：$obfs \\n服务器认证密码auth：$auth \\n本地socks5监听端口：$listen_socks5 \\n本地tproxy透明代理端口：$listen_tproxy \\n最大上行带宽up：$up \\n最大下行带宽down：$down\\n开机自启wan: $wan \\n定时启动cron: $cron \e[0m"\\n
echo "server_domain=$server_domain
server_port=$server_port
auth=$auth
obfs=$obfs
transport=$transport
listen_socks5=$listen_socks5
listen_tproxy=$listen_tproxy
up=$up
down=$down
wan=$wan
cron=$cron
...
##缺省参数补全
[ -z "$server_port" ] && server_port=443 && echo "server_port=$server_port" >> $dirconf/settings.txt
[ -z "$transport" ] && transport=udp && echo "transport=$transport" >> $dirconf/settings.txt
[ -z "$listen_socks5" ] && listen_socks5=9005 && echo "listen_socks5=$listen_socks5" >> $dirconf/settings.txt
[ -z "$listen_tproxy" ] && listen_tproxy=9001 && echo "listen_tproxy=$listen_tproxy" >> $dirconf/settings.txt
[ -z "$up" ] && up=20 && echo "up=$up" >> $dirconf/settings.txt
[ -z "$down" ] && down=100 && echo "down=$down" >> $dirconf/settings.txt
[ -z "$hop_interval" ] && hop_interval=30 && echo "hop_interval=$hop_interval" >> $dirconf/settings.txt
[ -z "$fast_open" ] && fast_open=true && echo "fast_open=$fast_open" >> $dirconf/settings.txt
...
if [ ! -z "$2" -a ! -z "$3" ] ; then
	#一键快速设置参数：./hysteria.sh 1 server_domain=domain.com server_port=443 auth=123456
...
run="$dirtmp/${name} -c $dirtmp/config.yaml"
...
version="$dirtmp/${name} version 2>&1 | grep -i version"
...
down_config () {
echo "▶生成配置文件config.yaml"
[ -z "$server_domain" ] && echo -e \\n"\e[1;31m✖ 服务器域名 server_domain 不存在，请重置初始化配置。结束脚本。\e[0m"\\n && exit
[ -z "$server_port" ] && echo -e \\n"\e[1;31m✖ 服务器端口 server_port 不存在，请重置初始化配置。结束脚本。\e[0m"\\n && exit
[ -z "$obfs" -a -z "$auth" ] && echo -e \\n"\e[1;31m✖ 服务器混淆密码 obfs 与 认证密码auth password都为空，需要至少有其中一项认证参数，请重置初始化配置。结束脚本。\e[0m"\\n && exit
cat > ./config.yaml << EOF
server: $server_domain:$server_port
auth: $auth
bandwidth:
  up: $up m
  down: $down m
...
lazy: true
socks5:
  listen: 0.0.0.0:$listen_socks5
tcpTProxy:
  listen: 0.0.0.0:$listen_tproxy
udpTProxy:
  listen: 0.0.0.0:$listen_tproxy
  timeout: 20s
...
[ "$(sysctl net.core.rmem_max | awk '{print $3}')" != "2500000" ] && echo "▶调整内核UDP缓存大小net.core.rmem_max、 net.core.wmem_max..." && sysctl -w net.core.rmem_max=2500000 && sysctl -w net.core.wmem_max=2500000
...
if [ ! -z "$server_domain" ] ; then
	echo -e "★ \e[1;36m 服务器域名domain：\e[1;32m $server_domain\e[0m"
else
	echo -e "☆ \e[1;36m 服务器域名domain：\e[1;31m【不存在】\e[0m"
fi
if [ ! -z "$server_port" ] ; then
	echo -e "★ \e[1;36m 服务器端口  port：\e[1;32m $server_port\e[0m"
else
	echo -e "☆ \e[1;36m 服务器端口  port：\e[1;31m【不存在】\e[0m"
fi
if [ ! -z "$auth" ] ; then
	echo -e "★ \e[1;36m 服务器认证码auth：\e[1;32m $auth\e[0m"
else
	echo -e "☆ \e[1;36m 服务器认证码auth：\e[0m\e[37m【无】\e[0m"
fi
if [ ! -z "$obfs" ] ; then
	echo -e "★ \e[1;36m 服务器混淆码obfs：\e[1;32m $obfs\e[0m"
else
	echo -e "☆ \e[1;36m 服务器混淆码obfs：\e[0m\e[37m【无】\e[0m"
fi
if [ ! -z "$transport" ] ; then
	echo -e "★ \e[1;36m 传输协议transport：\e[1;32m $transport\e[0m"
else
	echo -e "☆ \e[1;36m 传输协议transport：\e[0m\e[37m【空】\e[0m"
fi
pss_status=$(pss | wc -l)
...
if [ ! -z "$(port)" ] ; then
	echo -e "● \e[1;36m ${name} 端口：\e[1;32m【已监听】\e[0m socks5：\e[1;37m$listen_socks5\e[0m tproxy：\e[1;37m$listen_tproxy\e[0m "
else
